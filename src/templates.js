    ...
    />
      <div class="editor__metadata">
          ...
          <div class="metadata__field">
              ...
              <label for="is_public" class="metadata__field__label text-preset-5" style="cursor: pointer">Public</label>
              <input type="checkbox" id="is_public" name="is_public" ${isPublic ? "checked" : ""} style="cursor: pointer; width: 16px; height: 16px; accent-color: var(--blue-500);"/>
          </div>
          <div class="metadata__field">
              <svg
                  width="24"
...
export function sidebarTagListTemplate(tags = []) {
  if (!Array.isArray(tags) || !tags.length) {
    return `
      ...
      <ul id="tag-list" class="sidebar__menu">
        <li class="sidebar__menu__item text-muted text-preset-6">No tags yet</li>
      </ul>
    `;
  }
  ...
  const map = tags.reduce((acc, { name, count }) => {
    ...
    else acc[key].count += Number(count) || 0;
    return acc;
  }, {});

  const uniqueTags = Object.values(map).sort((a, b) =>
            ...
            <li class="sidebar__menu__item">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                  ...
                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9.907 8.315a1.607 1.607 0 0 1-1.61 1.583c-.872-.002-1.599-.73-1.594-1.596a1.604 1.604 0 0 1 1.633-1.607c.864.003 1.575.736 1.571 1.62Z" clip-rule="evenodd"/>
                </svg>
                <span class="sidebar__menu__label">${escapeHtml(name)}</span>
                <span class="text-muted" style="margin-left:auto">${Number(count)}</span>
              </button>
            </li>
          `,
        )
